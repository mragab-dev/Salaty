...
const getMockPrayerArray = (): Prayer[] => {
  ...
  return [
    { name: 'fajr', time: FALLBACK_MOCK_TIMES_OBJECT.fajr, arabicName: 'الفجر', date: dateStr },
    { name: 'sunrise', time: '06:00 AM', arabicName: 'الشروق', date: dateStr }, // Mock sunrise
    { name: 'dhuhr', time: FALLBACK_MOCK_TIMES_OBJECT.dhuhr, arabicName: 'الظهر', date: dateStr },
    { name: 'asr', time: FALLBACK_MOCK_TIMES_OBJECT.asr, arabicName: 'العصر', date: dateStr },
    { name: 'maghrib', time: FALLBACK_MOCK_TIMES_OBJECT.maghrib, arabicName: 'المغرب', date: dateStr },
    { name: 'isha', time: FALLBACK_MOCK_TIMES_OBJECT.isha, arabicName: 'العشاء', date: dateStr },
  ];
};
...
export const calculateAndFormatPrayerTimes = async (coords: RNGeolocationCoordinates | null): Promise<{ prayers: Prayer[], date: string, error?: string }> => {
  if (!coords) {
    console.warn("Location not provided, using mock prayer times.");
    const mockPrayers = getMockPrayerArray();
    return { prayers: mockPrayers, date: mockPrayers[0].date || new Date().toLocaleDateString('ar-EG'), error: "Location permission denied or failed. Displaying approximate times." };
  }
  ...
  try {
    ...
    const prayerTimes: Prayer[] = BASE_PRAYERS.map(p => {
      let time;
      switch (p.name) {
        case 'fajr':    time = moment(adhanPrayers.fajr).tz(timezone).format('hh:mm A'); break;
        case 'sunrise': time = moment(adhanPrayers.sunrise).tz(timezone).format('hh:mm A'); break;
        case 'dhuhr':   time = moment(adhanPrayers.dhuhr).tz(timezone).format('hh:mm A'); break;
        case 'asr':     time = moment(adhanPrayers.asr).tz(timezone).format('hh:mm A'); break;
        case 'maghrib': time = moment(adhanPrayers.maghrib).tz(timezone).format('hh:mm A'); break;
        case 'isha':    time = moment(adhanPrayers.isha).tz(timezone).format('hh:mm A'); break;
        default:        time = 'N/A';
      }
      return { ...p, time, date: formattedDate };
    });
    
    console.log('Formatted prayer times for timezone', timezone, prayerTimes);
    return { prayers: prayerTimes, date: formattedDate };

  } catch (error: any) {
    console.error('Error calculating prayer times with AdhanJS:', error);
    const mockPrayers = getMockPrayerArray();
    return { prayers: mockPrayers, date: mockPrayers[0].date || new Date().toLocaleDateString('ar-EG'), error: "Failed to calculate prayer times. Displaying approximate times." };
  }
};
